...
function TrendPage() {
  ...
  return (
    <div className="w-full p-4 text-white rounded-lg h-fit flex flex-col gap-y-8">
      {trend && (
        <>
            ...
            blogTrend.map(
              ({ id, blog }) =>
                blog && (
                  <div
                    key={id}
                    className="w-full p-4 rounded-md bg-slate-900 flex items-start gap-x-4 max-md:flex-col max-md:gap-y-4"
                  >
                    {blog.imgUrl && (
                      <div className="img w-48 h-48 max-md:w-full max-md:h-[400px]">
                        <img
                          src={blog.imgUrl}
